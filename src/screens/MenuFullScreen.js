...
import {
  ...
  SectionList, FlatList, Alert, RefreshControl, Platform
} from 'react-native';
...
export default function MenuFullScreen() {
  ...
  const toSections = (data) => {
    ...
    if (Array.isArray(data)) {
      return [{ key: 'all', title: 'Menu', data: data.map((it, k) => mapItem(it, null, k)) }];
    }
    if (Array.isArray(data?.items)) {
      return [{ key: 'all', title: 'Menu', data: data.items.map((it, k) => mapItem(it, null, k)) }];
    }
    return [];
  };
  ...
  const applyAndCache = async (data, shouldCache = true) => {
    ...
    setLastUpdated(now);
    if (shouldCache) {
      await AsyncStorage.setItem(CACHE_KEY, JSON.stringify({ ts: now, data }));
    }
  };
  ...
  const readCacheOnce = useCallback(async () => {
    try {
      ...
      if (ts && Date.now() - new Date(ts).getTime() < CACHE_TTL_MS) {
        setLoading(false);
        return true;
      }
    } catch {}
  ...
  useEffect(() => {
    (async () => {
      await readCacheOnce();
      fetchMenu({ force: false });
    })();
  }, [readCacheOnce, fetchMenu]);
  ...
  const onSelectTab = (key) => {
    ...
    if (idx > -1 && listRef.current) {
      try {
        listRef.current.scrollToLocation({ sectionIndex: idx, itemIndex: 0, animated: true, viewOffset: 6 });
      } catch {}
    }
  };
  ...
  const onAdd = (item) => {
    ...
    Alert.alert('Added', `${item.name} added to cart`, [{ text: 'OK' }], { cancelable: true });
  };
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <View style={styles.card}>
        ...
        >
          <Text style={[styles.addTxt, inCart && styles.addTxtActive]}>
            {inCart ? 'Added' : 'Add'}
          </Text>
        </TouchableOpacity>
      </View>
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={{ color: BRAND.muted, marginTop: 6 }}>Loading menu…</Text>
      </View>
    );
  }
  if (error) {
    return (
      <View style={styles.center}>
        ...
        <TouchableOpacity style={[styles.btn, styles.btnGhost, { marginTop: 10 }]} onPress={() => fetchMenu({ force: true })}>
          <Text style={styles.btnGhostTxt}>Retry</Text>
        </TouchableOpacity>
      </View>
    );
  }
    ...
    <View style={styles.wrap}>
      ...
      <View style={styles.statusRow}>
        <Text style={styles.statusTxt}>
          {lastUpdated ? `Updated ${new Date(lastUpdated).toLocaleTimeString()}` : 'Fresh'}
        </Text>
      </View>
      ...
      <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom + 10, 14) }]}>
        <View style={styles.sumRow}>
          <Text style={styles.totalLbl}>Cart</Text>
          <Text style={styles.totalVal}>
            {count} item{count === 1 ? '' : 's'} • R {Number(total || 0).toFixed(2)}
          </Text>
        </View>
        <View style={{ flexDirection: 'row', gap: 10 }}>
          ...
          >
            <Text style={styles.btnGhostTxt}>View cart</Text>
          </TouchableOpacity>
          <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  badge: {
    ...
    backgroundColor: BRAND.accent, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 4,
  },
  ...
  card: {
    flexDirection: 'row',
    gap: 12,
    borderWidth: 1, borderColor: BRAND.paperSoft,
    borderRadius: 14,
    padding: 12,
    backgroundColor: BRAND.paper,
    ...shadow,
  ...
  footer: {
    ...
    backgroundColor: BRAND.paper,
    borderTopWidth: 1, borderTopColor: BRAND.paperSoft,
